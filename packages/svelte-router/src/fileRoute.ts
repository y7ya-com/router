...
export class LazyRoute<TRoute extends AnyRoute> {
  ...
  useMatch: UseMatchRoute<TRoute['id']> = (opts) =>
    useMatch({ select: opts?.select, from: this.options.id } as any) as any

  useRouteContext: UseRouteContextRoute<TRoute['id']> = (opts) =>
    useRouteContext({ ...(opts as any), from: this.options.id }) as any

  useSearch: UseSearchRoute<TRoute['id']> = (opts) =>
    useSearch({ select: opts?.select, from: this.options.id } as any) as any

  useParams: UseParamsRoute<TRoute['id']> = (opts) =>
    useParams({ select: opts?.select, from: this.options.id } as any) as any

  useLoaderDeps: UseLoaderDepsRoute<TRoute['id']> = (opts) =>
    useLoaderDeps({ ...opts, from: this.options.id } as any)

  useLoaderData: UseLoaderDataRoute<TRoute['id']> = (opts) =>
    useLoaderData({ ...opts, from: this.options.id } as any)
...
>(id: ConstrainLiteral<TId, RouteIds<TRouter['routeTree']>>) {
  return (opts: LazyRouteOptions) => {
    return new LazyRoute<TRoute>({ id: id as string, ...opts })
  }
}
...
>(id: TFilePath): (opts: LazyRouteOptions) => LazyRoute<TRoute> {
  if (typeof id === 'object') {
    return new LazyRoute<TRoute>(id as any) as any
  }
